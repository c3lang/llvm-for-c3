...
then
  echo "Usage: $0 <llvm-version> <llvm-repository-url> <target-id> [Release/Debug]"
  exit 1
fi
...
# only on Windows (conflicts with Shared Libs on Linux)
if [[ "$OS_TYPE" == "windows" ]]; then
  CMAKE_ARGUMENTS="$CMAKE_ARGUMENTS -DLLVM_TOOL_LLVM_DRIVER_BUILD=ON"
else
  CMAKE_ARGUMENTS="$CMAKE_ARGUMENTS -DLLVM_TOOL_LLVM_DRIVER_BUILD=OFF"
fi
